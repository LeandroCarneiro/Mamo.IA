...
original_feature_names = {index: value for index, value in enumerate(df_cancer[0])}
...
modes = [
    ...
    # {
    #     'Name': 'LightGBM',
    #     'Model': MyXGboost.LightGBMMulticlass(X_train, y_train)
    # },
    # {
    #     'Name': 'AdaBoost',
...
for m in modes:
    ...
    if hasattr(model, 'feature_importances_'):
        ...
        # Save a new CSV file with only the top 10 features
        if 'feature_importance_df' in locals():
            # Get the top 10 features
            top_10_features = feature_importance_df.head(10)['Feature'].tolist()

            # Filter the original dataframe to include only the top 10 features
            top_10_data = df_cancer[top_10_features + ['Tag']]  # Include the target column

            # Save the filtered data to a new CSV file
            top_10_data.to_csv('top_10_features_data.csv', index=False)
            print("CSV file with top 10 features saved as 'top_10_features_data.csv'.")
