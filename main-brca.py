...
from helpers.datasetHelper import get_samples, split_healthy_data, split_healthy_data_HxC, split_healthy_data_MtxWt
from imblearn.over_sampling import SMOTE
from sklearn.metrics import cohen_kappa_score

from helpers.ploting import display_confusion_matrix_pink_variants, display_confusion_matrix_pink_variants_mt_wt
...
data_health = get_samples(os.path.join(directory_path, 'Mutation-WithoutMut.csv'))

# Load the PAN-CANCER-TRANSPOSED.csv data
healthy_cases, brca_cases = split_healthy_data_HxC(data_health)
...
df_cancer = pd.concat([healthy_cases, brca_cases], ignore_index=True) #blood samples
...
smote = SMOTE(sampling_strategy='auto', random_state=None, k_neighbors=3)
